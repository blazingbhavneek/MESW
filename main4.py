...
MAX_TOKEN = 2000
DEFINITION_SIMILARITY_THRESHOLD = 0.7  # If definitions are > threshold, consider them similar
...
class TermDefinition(BaseModel):
    ...
    definition: str = Field(..., description="Clear, concise definition of the term (1-2 sentences)")
...
class ChunkProcessor:
    ...
    async def process_chunk(self, chunk: Dict[str, Any], 
                           given_terms: Dict[str, str]) -> Dict[str, Any]:
        """
        ...
        definitions = {}
        if present_terms:
            definitions = await self._generate_chunk_definitions(chunk_text, present_terms, chunk_id)
...
class GraphRAGBuilder:
    ...
    def _compile_term_contexts(self, chunks: List[Dict[str, Any]], 
                              chunk_processor: ChunkProcessor) -> None:
    ...
    def save_results(self) -> None:
        ...
        with open(contexts_path, 'w', encoding='utf-8') as f:
            json.dump({k: list(v) for k, v in self.graph_manager.entity_contexts.items()}, 
...
class BookDictionaryQuery:
    ...
    def find_similar_terms(self, term: str, hops: int = 1, max_terms: int = 10) -> List[Dict[str, Any]]:
        """
    ...
    def format_enhanced_query_result(self, result: Dict[str, Any]) -> str:
        ...
        if result['similar_terms']:
            ...
            for i, term_info in enumerate(result['similar_terms'], 1):
                hop_label = "Original" if term_info['is_original'] else f"Hop {term_info['hop']}"
...
async def main():
    ...
    if command == "build":
        ...
        if success:
            ...
            print(f"   📚 Entity definitions saved to {os.path.join(WORKING_DIR, 'entity_definitions.json')}")
            print(f"   📌 Entity contexts saved to {os.path.join(WORKING_DIR, 'entity_contexts.json')}")
            print(f"   📊 Statistics saved to {os.path.join(WORKING_DIR, 'processing_stats.json')}")
        else:
        ...
        if not query_system.vocabulary:
            print("❌ No vocabulary loaded. Please build the graph first with: python script.py build [book_file]")
            return

...
class JsonToMarkdown:
    ...
    def _load_json(self) -> List[Dict[str, Any]]:
        ...
        sorted_data = sorted(data, key=lambda x: (x['page'], x['section'], x['subsection']))
    ...
    def _format_section_header(self, block: Dict[str, Any]) -> str:
        ...
        if block['subsection'] > 0 and block['subsection_name']:
            lines.append(f"**Subsection {block['subsection']}: {block['subsection_name']}**")
    ...
    def convert_simple(self, output_path: str = None) -> str:
        ...
        for block in self.data:
            ...
            if block['section'] != current_section and block['section'] > 0 and block['section_name']:
                current_section = block['section']
                current_subsection = None  # Reset subsection when section changes
            
            # Add subsection header if it's new
            if block['subsection'] != current_subsection and block['subsection'] > 0 and block['subsection_name']:
    ...
    def convert_detailed(self, output_path: str = None) -> str:
        ...
        markdown_lines.append("*This view shows the complete structure with metadata*\n")
        ...
        for i, block in enumerate(self.data, 1):
            ...
            markdown_lines.append("```")
            markdown_lines.append(f"Page: {block['page']}")
            markdown_lines.append(f"Section: {block['section']} - {block['section_name']}")
            markdown_lines.append(f"Subsection: {block['subsection']} - {block['subsection_name']}")
            markdown_lines.append("```\n")
        ...
        for block in self.data:
            ...
            if block['subsection'] != current_subsection:
                ...
                if block['subsection_name']:
                    ...
                    markdown_lines.append(f"*Subsection {block['subsection']} | Page {block['page']}*\n")
    ...
    def convert_toc(self, output_path: str = None) -> str:
        ...
        for block in self.data:
            if block['section'] > 0 and block['section_name']:
                ...
                if section_key not in seen_sections:
                    ...
                    markdown_lines.append(f"- [{block['section']}. {block['section_name']}](#{anchor}) (Page {block['page']})")
                    
                    # Add subsections under this section
                    for sub_block in self.data:
                        if (sub_block['section'] == block['section'] and 
                            sub_block['subsection'] > 0 and 
                            sub_block['subsection_name']):
                            subsection_key = (sub_block['section'], sub_block['subsection'], sub_block['subsection_name'])
                            if subsection_key not in seen_subsections:
                                seen_subsections.add(subsection_key)
                                sub_anchor = sub_block['subsection_name'].lower().replace(' ', '-')
                                markdown_lines.append(f"  - [{block['section']}.{sub_block['subsection']}. {sub_block['subsection_name']}](#{sub_anchor}) (Page {sub_block['page']})")
        ...
        for block in self.data:
            ...
            if block['section'] != current_section:
                ...
                if block['section_name']:
                    markdown_lines.append(f"\n# {block['section_name']}")
                    markdown_lines.append(f"*Section {block['section']} | Page {block['page']}*\n")
